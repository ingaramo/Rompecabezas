...
var instrucciones = ['Debes usar las flechas para mover la pieza vacia'];
...
function intercambiarPosicionesGrilla(filaPos1, columnaPos1, filaPos2, columnaPos2) {
    let cambio1= grilla[filaPos1][columnaPos1];
    let cambio2= grilla[filaPos2][columnaPos2];
    grilla[filaPos2][columnaPos2]= cambio1;
    grilla[filaPos1][columnaPos1]= cambio2;
}
...
function mezclarPiezas(veces) {
  ...
  var direccion = direcciones[Math.floor(Math.random() * direcciones.length)];//realmente es random? hay muy pocas posibilidadees de que salga 3
  console.log(direccion) ; 
  moverEnDireccion(direccion);

  setTimeout(function() {
      mezclarPiezas(veces - 1); // por que no llamar a la funcino directamente como parametr setTimeOut(mezclarPiezas(veces-1),100)
    },0);
}
